...
(function(root){
'use strict';
const P=BondProfile,W=BondWorld;let category='All',selected='biscuit';
...
function render(){
 ...
 if(item){
  ...
  if(item.category==='Echoes'){
   const origin=s.echoes[item.type]?.[0];detail+='<p class="item-note">Source: '+(origin?BondAtlas.get(origin.map).name+' · Lv '+origin.level:'saved reward')+'</p><button class="button primary '+(selected===tutorialEcho?'tutorial-target':'')+'" data-summon="'+item.type+'" '+''+'>'+'Summon a new individual'+'</button>';
  ...
  else detail+='<p class="item-note">A keepsake, not equipment. No hidden stat bonuses or current crafting use.</p>';
 }
 return '<div class="satchel-heading"><div><p class="eyebrow">THE THINGS WE CARRY</p><h3>Your travel satchel</h3><p>Earned in the world. Kept safe between adventures.</p></div><span class="satchel-coins"><b>'+s.coins+'</b> trail coins</span></div><div class="satchel-layout"><aside class="satchel-categories" aria-label="Item categories">'+['All','Echoes','Supplies','Materials','Trophies','Legacy'].map(c=>'<button data-bag-filter="'+c+'" class="'+(category===c?'selected':'')+'" aria-pressed="'+(category===c)+'"><span>'+({All:'◈',Echoes:'✧',Supplies:'✦',Materials:'◇',Trophies:'♛',Legacy:'❧'}[c])+'</span>'+c+'<small>'+Object.keys(W.ITEMS).filter(k=>s.inventory[k]>0&&(c==='All'||W.ITEMS[k].category===c)).length+'</small></button>').join('')+'<p>No weight limit. No paid gameplay items.</p></aside><section class="satchel-storage"><div class="bag-grid-heading"><b>'+category.toUpperCase()+'</b><small>'+ids.length+' item types</small></div><div class="satchel-grid">'+ids.map(k=>'<button class="satchel-slot inventory-item '+(selected===k?'selected':'')+(k===tutorialEcho?' tutorial-target':'')+'" data-item="'+k+'" aria-pressed="'+(selected===k)+'" aria-label="'+W.ITEMS[k].name+', '+s.inventory[k]+' owned">'+icon(k)+'<b class="stack-count">×'+s.inventory[k]+'</b><strong>'+W.ITEMS[k].name+'</strong><small>'+W.ITEMS[k].category+'</small></button>').join('')+Array.from({length:Math.max(0,8-ids.length)},()=>'<span class="satchel-empty-slot" aria-hidden="true">✧</span>').join('')+'</div>'+(!ids.length?'<p class="bag-empty-message">Nothing in this category yet. Wild kills can leave Soul Echoes; previously earned drops survive later defeats.</p>':'')+'<div class="earned-supplies"><h4>Restock in the village</h4><p>Click the Supply Store trade tent for cheap recovery items. The sanctuary restores everyone for free.</p><button class="button secondary" data-open-recovery>Use recovery items</button></div></section><aside class="satchel-detail item-detail">'+detail+'</aside></div>';
}
document.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;if(b.dataset.bagFilter){category=b.dataset.bagFilter;BondMenu.render();document.querySelector('[data-bag-filter="'+category+'"]')?.focus({preventScroll:true});}if(b.hasAttribute('data-pick-feed'))BondPicker.open({title:'Feed one companion · +120 XP',returnSelector:'[data-pick-feed]',onChoose:id=>P.feed(id)});if(b.dataset.buy){P.buy(b.dataset.buy);document.querySelector('[data-buy="'+b.dataset.buy+'"]')?.focus({preventScroll:true});}});
root.BondInventory={render,icon,select(id){if(W.ITEMS[id]){selected=id;category=W.ITEMS[id].category==='Echoes'?'Echoes':'All';}},openEcho(){const s=P.snapshot(),id=Object.keys(s.inventory).find(k=>k.startsWith('echo:')&&s.inventory[k]>0);if(!id)return;selected=id;category='Echoes';BondApp.switchTab('loadout');BondMenu.open('inventory');}};
})(globalThis);
