...
function trainer(){
 ...
 return '<section class="trainer-ledger"><div class="ledger-heading"><div>'+CharacterRig.art(type)+'</div><section><p class="eyebrow">THE SUMMONER</p><h3>Level '+l+' '+BondContent.UNITS[type].name+'</h3><progress max="'+need+'" value="'+(atCap?need:into)+'" aria-label="Trainer level progress"></progress><small>'+(atCap?'Max level':into+' / '+need+' XP to level '+(l+1))+'</small><strong id="attribute-points">'+left+' attribute points available</strong></section></div><div class="attribute-grid">'+R.ATTRS.map(k=>'<article><div><small>'+k.toUpperCase()+'</small><b>'+a[k]+'</b><p>'+descriptions[k]+'</p></div><button class="button secondary" data-stat="'+k+'" '+(a[k]>=99||left<R.cost(a[k])?'disabled':'')+'>+1 · '+R.cost(a[k])+' pts</button></article>').join('')+'</div><button data-stat-reset class="text-button">Reset attributes · free</button><div class="leadership-preview"><h4>Leadership · '+(a.leadership*.5).toFixed(1)+'% shared</h4><p>Each active companion receives '+R.ATTRS.slice(0,5).map(k=>'+'+(a[k]*a.leadership*.005).toFixed(2)+' '+k.toUpperCase()).join(' · ')+'.</p></div><h4>Companion experience</h4><div class="xp-list">'+s.companions.map(mon=>{const t=mon.type,xp=mon.xp,n=R.monLevel(s,mon.id),atCap=n===R.PLAYER_LEVEL_CAP,into=xp-R.threshold(n),need=atCap?1:R.threshold(n+1)-R.threshold(n);return '<article><span>'+CharacterRig.art(t)+'</span><div><strong>'+P.label(mon)+' · Lv '+n+'</strong><small>'+(atCap?'Max level':into+' / '+need+' XP to level '+(n+1))+'</small><progress max="'+need+'" value="'+(atCap?need:into)+'" aria-label="'+BondContent.UNITS[t].name+' level progress"></progress></div></article>';}).join('')+'</div><h4>Four elements</h4><p>Water → Fire → Earth → Wind → Water. Advantage ×1.2 · Disadvantage ×0.8.</p><div class="element-table"><table><caption>Damage multiplier · rows attack, columns defend</caption><thead><tr><th scope="col">Element</th>'+R.ELEMENTS.map(x=>'<th scope="col">'+x+'</th>').join('')+'</tr></thead><tbody>'+R.ELEMENTS.map(a=>'<tr><th scope="row">'+a+'</th>'+R.ELEMENTS.map(d=>'<td>×'+R.multiplier(a,d).toFixed(1)+'</td>').join('')+'</tr>').join('')+'</tbody></table></div></section>';
}
function result(b,id){
 const r=P.complete(b,id);if(!r)return;
 if(r.pending){const p=document.createElement('p');p.className='reward-summary';p.textContent=r.error+'';const retry=document.createElement('button');retry.className='button primary';retry.textContent='Retry saving';retry.onclick=()=>BondApp.finish();$('#result').append(p,retry);return;}
