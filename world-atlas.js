...
const point=m=>({x:PAD+(m.grid.x+.5)*SIZE,y:PAD+(m.grid.y+.5)*SIZE});
...
function roadsMarkup(){const seen=new Set(),roads=[];for(const m of A.maps)for(const g of m.neighbors){const key=[m.id,g.to].sort().join('|');if(seen.has(key))continue;seen.add(key);const a=point(m),b=point(A.get(g.to));roads.push('<path data-atlas-road="'+key+'" d="M'+a.x+' '+a.y+'L'+b.x+' '+b.y+'"/>');}return roads.join('');}
function render(container){
 ...
 const places=A.maps.map(m=>{const p=point(m),here=m.id===s.map,visited=s.visited.includes(m.id),locked=!A.unlocked(s,m.id);return '<button class="atlas-place '+m.kind+' '+(here?'here ':'')+(visited?'visited ':'')+(locked?'locked ':'')+'" data-atlas-select="'+m.id+'" data-kind="'+m.kind+'" style="left:'+(p.x/EXTENT*100)+'%;top:'+(p.y/EXTENT*100)+'%;--cell-color:'+m.colors[0]+'" aria-pressed="'+(m.id===selected)+'" aria-label="'+m.name+', '+(here?'you are here':visited?'visited':'unexplored')+'"><small>'+kindLabel(m)+'</small><strong>'+m.name+'</strong><span>'+(here?'You are here':locked?'Road closed':m.kind==='hub'?BondCities.theme(m).title:visited?'Visited':'Unexplored')+'</span><i class="atlas-cell-coordinate">'+String.fromCharCode(65+m.grid.x)+(m.grid.y+1)+'</i></button>';}).join('');
