...
const profiles={
 ...
 anthill:{name:'Living Anthill',cap:1,hp:.1,life:6},
 ...
 'seed-bomb':{name:'Seed Bomb',cap:1,hp:.07,life:2,pulses:[1],reach:'owner'},
...
function spawn(f,u,id,options={}){
 const base=profiles[id];if(!base||!alive(u)||f.battle.ended)return null;const p=f.battle.monsterRules?{...base,...(base.pulses?{pulses:[...base.pulses]}:{})}:base;
 const group=p.capGroup||id,count=f.entities.filter(e=>alive(e)&&e.master===u&&e.capGroup===group).length;
 const cap=root.BondCompanionTalents?.own(f,'entityCap',u,id,p.cap)??p.cap;if(count>=cap)return null;
 ...
 const b=f.battle,e={id:'entity-'+(++f.sequence),type:u.type,name:p.name,profile:id,capGroup:group,master:u,owner:u.owner,ownerIndex:u.ownerIndex,side:u.side,slot:1000+f.sequence,temporary:true,encounterActor:false,
  snapshot:stats,spec:p,hp,maxHp:hp,position,previousPosition:{...position},level:u.level,effective:{dex:Math.max(0,stats.hit-u.level),agi:0,vit:0,int:0},
...
function covered(f,e){const p=e.spec||profiles[e.profile];return f.core(e.master).filter(u=>!p.radius||f.battle.distance(e,u)<=p.radius+.001);}
function target(f,e){
 const enemies=f.enemies(e),p=e.spec||profiles[e.profile],ownerTarget=f.battle.target(e.master);
 if(f.battle.monsterRules&&e.profile==='soldier-ant'){const locked=enemies.find(t=>t.id===e.targetId);if(locked)return locked;}
 const legal=enemies.filter(t=>f.battle.distance(e,t)<=e.entityReach+.001);
 if(f.battle.monsterRules&&e.profile==='seedjaw')return legal.find(t=>t.id===e.targetId)||legal.sort((a,b)=>f.battle.distance(e,a)-f.battle.distance(e,b)||a.id.localeCompare(b.id))[0]||null;
...
function pulse(f,e,override={}){
 if(!alive(e)||!alive(e.master)||f.battle.ended)return;const p=e.spec||profiles[e.profile],b=f.battle;
 if(root.BondCompanionTalents?.own(f,'entityPulse',e.master,e,override))return;
...
function step(f){
 ...
 for(const e of [...f.entities]){
  ...
  const p=e.spec||profiles[e.profile];e.previousPosition={...e.position};
  ...
  root.BondCompanionTalents?.own(f,'entityTick',e.master,e);
  if(p.radius&&['light-beacon','assault-rune'].includes(e.profile))for(const u of covered(f,e)){
   if(e.profile!=='assault-rune'||!root.BondCompanionTalents?.own(f,'runeAura',e.master,e,u))f.put(e.master,u,e.profile==='light-beacon'?'Beacon aim':'Rune aim','hit',e.profile==='light-beacon'?0:20,.1);
   if(e.profile==='light-beacon'&&!root.BondCompanionTalents?.own(f,'beaconAura',e.master,e,u))f.put(e.master,u,'Beacon light','basicDamage',.1,.1);
  }
  if(e.profile==='silk-anchor'&&!root.BondCompanionTalents?.own(f,'silkAura',e.master,e))for(const t of f.nearby(e,e,p.radius,100))f.put(e.master,t,'Crescent Web','basicPenalty',.12,.1,{harmful:true});
  if(e.profile==='imperial-hearth'&&!root.BondCompanionTalents?.own(f,'hearthAura',e.master,e)&&b.distance(e,e.master)<=p.radius)f.put(e.master,e.master,'Hearth shelter','dr',.1,.1);
...
function primary(f,u,t,d){
 ...
 for(const e of f.entities.filter(e=>alive(e)&&e.side===u.side&&e.owner===u.owner)){
  const p=e.spec||profiles[e.profile];
  if(e.profile==='stitched-effigy'&&e.linked===t.id&&root.BondCompanionTalents?.own(f,'effigyTrigger',e.master,e,t)===undefined&&e.triggers<p.triggers&&f.ready(e,'triggerCD',.75)){e.triggers++;f.proc(e,t,.3*e.snapshot.A,'melee','Stitched Effigy');}
  if(e.profile==='assault-rune'&&d.active&&f.battle.distance(e,u)<=p.radius&&root.BondCompanionTalents?.own(f,'runeTrigger',e.master,e,u,t,d)===undefined&&e.triggers<p.triggers&&f.ready(e,'triggerCD',1)){e.triggers++;f.proc(e,t,.25*e.snapshot.M,'magic','Assault Rune');}
 }
}
...
function lure(f,actor,target){const e=f.entities.filter(e=>alive(e)&&e.profile==='ink-double'&&e.linked===actor.id&&e.lureUntil>f.battle.time&&e.side!==actor.side&&!e.lureUsed&&f.battle.distance(actor,e)<=f.battle.reach(actor)).sort((a,b)=>a.id.localeCompare(b.id))[0];if(e){e.lureCount=(e.lureCount||0)+1;e.lureUsed=e.lureCount>=(e.lureLimit||1);root.BondCompanionTalents?.own(f,'entityLured',e.master,e,actor);return e;}return target;}
...
function interceptProjectile(f,actor,target,raw,d){
 ...
 if(!screens.length)return raw;const e=screens[0].e;
 const custom=root.BondCompanionTalents?.own(f,'screenImpact',e.master,e,actor,raw,d);if(custom!==undefined)return custom;
 const used=Math.min(e.hp,raw);f.loss(actor,e,used,'Resin Screen',{screen:true,direct:false});
