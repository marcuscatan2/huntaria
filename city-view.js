...
function resident(id){const s=P.snapshot(),m=A.get(s.map),r=m?.residents?.find(r=>r.id===id),p=root.BondRegion?.residentPosition(id)||r;if(!r||s.encounterSave||Math.hypot(s.position.x-p.x,s.position.y-p.y)>150)return false;if(r.service)return BondCompanionServices.open(id);room=null;dialog.innerHTML=header(m.name,r.name)+'<div class="city-conversation"><div>'+CharacterRig.art(r.appearance)+'</div><blockquote>'+escape(r.text)+'</blockquote>'+(r.pet?'<div>'+CharacterRig.art(r.pet)+'<small>'+escape(BondContent.UNITS[r.pet].name)+'</small></div>':'')+'</div>';dialog.classList.add('is-conversation');show();return true;}
function teleport(){
