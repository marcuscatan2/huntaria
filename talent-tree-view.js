...
function icon(type,index){const companion=BondGrowth.companionTree(type),atlas=companion?({Tank:'swordsman',Support:'druid',Ranged:'hunter'}[BondContent.UNITS[type].role]||(BondContent.UNITS[type].basicCategory==='magic'?'mage':'hunter')):type;index=companion?index%15:index;return '<span class="talent-icon" aria-hidden="true" style="--icon-x:'+(index%5*25)+'%;--icon-y:'+(Math.floor(index/5)*50)+'%;--icon-atlas:url(assets/talents/'+atlas+'.png)"></span>';}
...
function render(ctx){
 ...
 const companion=BondGrowth.companionTree(type),layout=companion?[[50,48],[24,172],[76,172],[24,296],[76,296],[50,448],[50,584],[50,740]]:positions,height=companion?814:508;
 ...
 const groupMarkup=groups.map((branch,bi)=>{
  ...
  const links=companion?group.flatMap((n,i)=>[...n.requires.all,...n.requires.any].map(id=>[group.findIndex(v=>v.id===id)+1,i+1])):paths;
...
function choose(type,id){selections[type]=id;const nodes=BondGrowth.nodes(type),index=nodes.findIndex(n=>n.id===id);if(index>=0)branches[type]=Math.floor(index/(BondGrowth.companionTree(type)?8:5));}
function branch(type,index){branches[type]=index;selections[type]=BondGrowth.nodes(type)[index*(BondGrowth.companionTree(type)?8:5)]?.id;}
function inspect(){
