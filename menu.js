...
function detail(type,entry=null,instanceId=null){
 ...
 (!preview?BondEquipmentView.quick(mon?.id||type):'')+'<button class="button secondary open-tree-button" data-open-tree="'+(mon?.id||type)+'">Mastery tree</button></div><div class="ability-panel"><div class="ability-heading"><div><h3>Combat Skills</h3></div>'+(editing?'<button class="text-button" data-rotate>Rotate order</button>':'')+'</div>'+
...
function collectionView(build){
 const switcher='<nav class="collection-mode" aria-label="Inner Sea areas">'+[['farm','Homestead'],['party','Party'],['equipment','Equipment'],['formation','Formation'],['trainer','Attributes'],['companions','My companions'],['catalog','Species guide']].map(([id,name])=>'<button '+(['party','equipment','formation','trainer'].includes(id)?'data-menu="'+id+'" ':'')+'data-collection-mode="'+id+'" aria-pressed="'+(collectionMode===id)+'" class="button secondary '+(collectionMode===id?'selected':'')+'">'+name+'</button>').join('')+'</nav>';
...
function open(name){
 if(!['party','equipment','collection','inventory','trees','trainer','formation','mastery'].includes(name))return;
 if(['party','equipment','trainer','formation','mastery'].includes(name)){tab='collection';collectionMode=name;}else{tab=name;if(name==='collection')collectionMode='farm';}
