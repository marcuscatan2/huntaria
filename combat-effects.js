...
class Effects{
 ...
 put(source,target,key,kind,value,duration,extra={}){
  ...
  const packet={source,target,key,kind,value,duration,extra};root.BondCompanionTalents?.each(this,'effect',packet);if(packet.cancel)return null;value=packet.value;duration=packet.duration;kind=packet.kind;extra=packet.extra;
  ...
  root.BondCompanionTalents?.each(this,'effected',source,target,e,old);
 ...
 add(u,key,n,max=Infinity){max=root.BondCompanionTalents?.change(this,'capacity',max,u,key)??max;return u.kit[key]=clamp((u.kit[key]||0)+n,0,max);}
 ...
 start(u){root.BondCompanionTalents?.own(this,'prepare',u);root.BondCombatPassives?.start(this,u);root.BondClassTalents?.start(this,u);root.BondCompanionTalents?.own(this,'start',u);}
 tick(){
  const b=this.battle;
  ...
  for(const u of b.units){
   this.syncShield(u);
   if(!alive(u))continue;
   for(const [key,e] of Object.entries(u.effects))if(e.until<b.time-1e-8||e.until<=b.time+1e-8&&e.kind!=='dot'){delete u.effects[key];if(e.expire)e.expire();root.BondCompanionTalents?.each(this,'effectExpired',u,e);}
   root.BondCombatPassives?.tick(this,u);
   root.BondClassTalents?.tick(this,u);
   root.BondCompanionTalents?.own(this,'tick',u);
  }
 ...
 syncShield(u){
  ...
  for(const p of expired)root.BondCompanionTalents?.each(this,'expired',u,p);
 }
 shield(source,target,amount,duration,label,options={}){
  if(!alive(target)||this.battle.ended)return 0;
  this.syncShield(target);
  const grant={source,target,amount,duration,label,options};root.BondCompanionTalents?.each(this,'shield',grant);amount=grant.amount;duration=grant.duration;
  ...
  const pool={key,label,source:source.id,amount:desired,capacity:desired,until:this.battle.time+duration,cast:source.casts||0,talent:!!options.talent,absorbed:0};
  ...
  root.BondCompanionTalents?.each(this,'shielded',source,target,granted,label,old||pool,options);
  return granted;
 }
 sharedShield(source,targets,amount,duration,label,options={}){
  const drafts=[],recipients=[...new Set(targets)].filter(alive);
  for(const target of recipients)this.shield(source,target,amount,duration,label,{...options,sharedDraft:drafts});
  ...
  for(const d of drafts){
   const p={key:d.key,label,source:source.id,amount:capacity,capacity,until:this.battle.time+d.duration,cast:source.casts||0,talent:!!options.talent,absorbed:0,shared};
  ...
  drafts.forEach((d,i)=>{
   ...
   root.BondCompanionTalents?.each(this,'shielded',source,d.target,Math.max(0,capacity-d.before),label,d.pool,options);
  });
  return granted;
 }
 absorb(target,raw,actor,details){
  ...
  for(const p of [...target.pools].sort((a,b)=>a.until-b.until||a.key.localeCompare(b.key))){
   ...
   const bonus=Math.max(0,details.shieldBonus||0),cost=details.active&&details.secondary?(p.shared?.splashCost||1):1,normal=Math.min(remaining,p.amount/cost),used=Math.min(p.amount,Math.round(normal*cost*(1+bonus)));
   ...
   p.absorbed=(p.absorbed||0)+normal;this.after(()=>root.BondCompanionTalents?.each(this,'absorbed',actor,target,p,normal,details,used));
   if(p.amount<=0&&actor&&actor.side!==target.side)this.after(()=>{root.BondCombatPassives?.broken(this,actor,target,p,details);root.BondClassTalents?.broken(this,actor,target,p,details);root.BondCompanionTalents?.each(this,'broken',actor,target,p,details);});
  }
 ...
 reduction(target,actor,raw,details){
  ...
  for(const e of Object.values(target.effects||{})){
   ...
   if(applies){multiplier*=1-e.value;if(e.once){delete target.effects[e.key];this.after(()=>root.BondCompanionTalents?.each(this,'defenseConsumed',target,e,actor,details));}}
  }
  ...
  raw=root.BondCompanionTalents?.change(this,'incoming',raw,actor,target,details)??raw;
 ...
 beforeHP(actor,target,raw,details){
  ...
  raw=root.BondCompanionTalents?.change(this,'beforeHP',raw,actor,target,details)??raw;
 ...
 heal(source,target,amount,label,options={}){
  ...
  tuned=root.BondCompanionTalents?.change(this,'healAmount',tuned,source,target,options,label)??tuned;
  ...
  root.BondCompanionTalents?.each(this,'healed',source,target,effective,label,{...options,offered,before});
  return effective;
 }
 direct(source,target,amount,label,options={}){
  ...
  const bonus=!options.proc?root.BondCombatPassives.hitBonus(this,source,target,options)+(root.BondClassTalents?.hitBonus(this,source,target,options)||0)+(root.BondCompanionTalents?.change(this,'hitBonus',0,source,target,options)||0)+(rolled?.hitBonus||0):0;
  ...
  if(!hit){const missed=shroud?{...options,shroudEvaded:shroud}:options;this.battle.emit('dodge',source,target,'Miss',0,{category});root.BondCombatPassives.missed(this,source,target,options);root.BondCompanionTalents?.each(this,'missed',source,target,missed);if(!options.proc&&options.primary!==false)root.BondCompanionTalents?.each(this,'attempted',source,target,options);return {hit:false,damage:0};}
  ...
  if(!options.proc)amount=root.BondCompanionTalents?.change(this,'outgoing',amount,source,target,details)??amount;
  ...
  if(source.temporary)this.after(()=>root.BondCompanionTalents?.own(this,'entityLanded',source.master,source,target,{hit:true,critical,...result},details));
  if(!details.proc&&details.primary)this.after(()=>{root.BondCombatPassives.landed(this,source,target,{hit:true,critical,...result},details);root.BondCompanionTalents?.each(this,'landed',source,target,{hit:true,critical,...result},details);});
  if(!details.proc&&details.primary)this.after(()=>root.BondCompanionTalents?.each(this,'attempted',source,target,details));
 ...
 managedDot(source,target,key,total,duration,category,options={}){
  ...
  const pulse=()=>{
   ...
   const result=this.battle.damage(source,target,current.tickAmount?current.tickAmount(current):current.value,current.label||key,false,{category,dot:true,direct:false,proc:true});
