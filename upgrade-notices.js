...
function read(s,type=s.progression?.specialization||'apprentice'){
 ...
 const inner=!!stats.size||[...companions.values()].some(t=>t.available);
 return {stats,attributePoints:left,trainer,companions,inner,any:inner||trainer.available};
}
function mark(button,active){
 ...
 if(on)button.setAttribute('aria-description','Upgrade points available');
 else if(button.getAttribute('aria-description')==='Upgrade points available')button.removeAttribute('aria-description');
}
function refresh(){
 ...
 const route=(name,entry)=>name==='inventory'?entry&&state.any:name==='collection'?state.inner:name==='trees'?state.trainer.available:name==='trainer'?!!state.stats.size:name==='companions'?[...state.companions.values()].some(t=>t.available):false;
 for(const button of document.querySelectorAll('[data-world-menu],[data-frame-menu],[data-menu],[data-collection-mode]'))mark(button,route(button.dataset.worldMenu||button.dataset.frameMenu||button.dataset.menu||button.dataset.collectionMode,!!(button.dataset.worldMenu||button.dataset.frameMenu)));
 for(const button of document.querySelectorAll('[data-instance]'))mark(button,state.companions.get(button.dataset.instance)?.available);
