...
function render(){const s=P.snapshot(),early=C.earlyNext(s),storyNext=C.chapters.flatMap(c=>c.steps).find(o=>!s.journey?.steps.includes(o.id))||null,chapter=C.chapters.find(c=>c.steps.some(o=>o.id===storyNext?.id)),cards=earlyCards(s);
 ...
 (early?'<section class="early-next"><small>CURRENT</small><h3>'+early.label+'</h3><p>'+A.get(early.map).name+' · Player Lv '+BondProgress.trainerLevel(s)+'</p><button class="button primary" data-early-map="'+early.map+'">Show the route →</button></section>':'')+
 ...
 dialog.querySelector('[data-early-map]')?.addEventListener('click',e=>{dialog.close();BondRegion.travelTo(e.currentTarget.dataset.earlyMap);});
