...
function render(){const s=P.snapshot(),early=C.earlyNext(s),storyNext=C.chapters.flatMap(c=>c.steps).find(o=>!s.journey?.steps.includes(o.id))||null,chapter=C.chapters.find(c=>c.steps.some(o=>o.id===storyNext?.id)),cards=earlyCards(s);
 dialog.innerHTML='<p class="eyebrow">YOUR JOURNEY</p><h2 id="campaign-title">'+(specTitle(s))+'</h2><p>'+(early?.complete?'Quest complete.':'')+'</p>'+
 ...
 '<details class="later-story"><summary>Optional regional stories - The Six Beacons</summary>'+C.chapters.map((c,i)=>'<details class="campaign-chapter" '+(c===chapter?'open':'')+'><summary>Chapter '+(i+1)+' · '+c.title+(s.journey.chapters.includes(c.id)?' ✓':'')+'</summary><p>'+c.intro+'</p><ol>'+c.steps.map(step=>{const done=s.journey.steps.includes(step.id),current=step===storyNext;return '<li class="'+(done?'complete':current?'current':'')+'"><span>'+(done?'✓ ':current?'→ ':'○ ')+step.label+' <small>'+Math.min(step.count,C.facts(s,step))+'/'+step.count+'</small></span><button class="text-button" data-story-map="'+step.map+'" '+(!A.unlocked(s,step.map)?'disabled':'')+'>Walk to '+A.get(step.map).name+'</button></li>';}).join('')+'</ol><p>Chapter reward: '+c.coins+' coins · '+c.xp+' XP. '+(s.journey.chapters.includes(c.id)?c.ending:'')+'</p></details>').join('')+'</details>'+
 '<h3>Optional regional recognition</h3><div class="challenge-grid">'+C.challenges.map(c=>{const n=C.challengeCount(s,c),done=s.journey.challenges.includes(c.id);return '<article><small>'+A.REGIONS.find(r=>r.id===c.region).name+' · '+c.scope+'</small><h4>'+c.label+'</h4><p>'+Math.min(n,c.count)+' / '+c.count+' · '+c.coins+' coins + decorative ribbon</p><button class="button secondary" data-challenge="'+c.id+'" '+(done||n<c.count?'disabled':'')+'>'+(done?'Claimed':'Claim recognition')+'</button></article>';}).join('')+'</div><button id="campaign-close" class="button primary">Back to the world</button>';
