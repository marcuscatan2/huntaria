...
transformButton.onclick=()=>{const e=P.encounter(dialogId);if(!e?.masterClass||!P.canSpecialize(e.masterClass))return;if(!confirm('Become a '+C.UNITS[e.masterClass].name+'? This commits your character to this class.'))return;const result=P.specialize(e.masterClass);if(result){$('#npc-dialog').close();party=BondApp.getBuild()[0];loadMap();message('Class transformed: '+C.UNITS[e.masterClass].name+'.');}};
