...
function buildObjects(){
 ...
 for(const e of [...BondCampaign.trainers,...BondCampaign.earlyEncounters.filter(e=>!e.kind)].filter(e=>e.map===m.id&&BondCampaign.visible(e,state)&&!(quiet()&&m.id==='clearing-0')))objects.push({id:e.id,kind:'npc',x:e.x,y:e.y,masterClass:e.masterClass,applicationClass:e.applicationClass,label:e.name+' · '+(e.masterClass?'CLASS TRIAL':e.main?'STORY':e.lesson||'CHALLENGE')});
...
function talk(id){
 ...
 $('#npc-reward').textContent=e.kind==='wild'?'Per kill: '+(6+Math.floor(e.level/3))+' coins and '+BondAdventure.xp(e.level)+' companion XP.':e.practice?'No rewards in this local boss preview.':P.snapshot().defeated.includes(id)?'Cleared. Rematches give no additional first-win rewards.':'First victory: '+(e.coins||0)+' coins, '+(e.trainerXP??e.xp??150)+' trainer XP, '+(e.xp||150)+' companion XP and a biscuit.';
 ...
 trialControls.hidden=!e.trialClass;if(e.trialClass){const u=C.UNITS[e.trialClass],defaults=[...u.default];trialControls.innerHTML='<p class="eyebrow">TEMPORARY '+u.name.toUpperCase()+' BUILD</p><p>Choose three abilities for this trial. Your Apprentice build will not change.</p>'+[0,1,2].map((slot)=>'<label>Priority '+(slot+1)+' <select data-trial-skill="'+slot+'">'+u.skills.map(skill=>'<option value="'+skill+'" '+(defaults[slot]===skill?'selected':'')+'>'+C.SKILLS[skill].name+'</option>').join('')+'</select></label>').join('');}
 bossControls.hidden=e.kind!=='boss'||!e.practice;$('#boss-test-level').value=P.snapshot().bossLevel;$('#npc-advice').hidden=e.kind==='wild'||e.openingGate;$('#npc-fight').hidden=!!gatePassed;$('#npc-fight').disabled=!P.validEncounter(id)||!!injury||!!requirement;$('#npc-fight').textContent=e.openingGate?'Begin trial →':e.practice?'Practice boss →':e.trialClass&&trialComplete?'Repeat class trial →':'Challenge →';
...
$('#npc-advice').onclick=()=>{$('#npc-dialogue').textContent=P.encounter(dialogId).advice;};
transformButton.onclick=()=>{const e=P.encounter(dialogId);if(!e?.masterClass||!P.canSpecialize(e.masterClass))return;if(!confirm('Become '+C.UNITS[e.masterClass].name+'? You will no longer switch classes from the loadout menu.'))return;const result=P.specialize(e.masterClass);if(result){$('#npc-dialog').close();party=BondApp.getBuild()[0];loadMap();message('Class transformed: '+C.UNITS[e.masterClass].name+'.');}};
$('#boss-match-level').onclick=()=>{$('#boss-test-level').value=BondProgress.trainerLevel(P.snapshot());};
$('#npc-fight').onclick=()=>{const id=dialogId,e=P.encounter(id),options={};if(!e){$('#npc-dialog').close();message('That encounter is no longer available.');return;}if(e.kind==='boss'&&e.practice){const input=$('#boss-test-level');if(!input.reportValidity())return;options.bossLevel=input.valueAsNumber;options.practiceParties=Number($('#boss-practice-parties').value);}if(e.trialClass){options.trialSkills=[...trialControls.querySelectorAll('[data-trial-skill]')].map(s=>s.value);if(new Set(options.trialSkills).size!==3){$('#npc-dialogue').textContent='Choose three different abilities for the trial.';return;}}$('#npc-dialog').close();if(!BondApp.startRegionBattle(id,options))message(P.requirement(id,party)||BondAdventure.readiness(P.snapshot(),party)||'Could not start. Resume the saved fight, or retry saving.');};
