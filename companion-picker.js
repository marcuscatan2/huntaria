...
function draw(focusSearch=false){
 ...
 dialog.innerHTML='<header><div><h2 id="picker-title">'+escape(options.title||'Choose your companion')+'</h2></div><button id="picker-close" class="text-button" aria-label="Close companion picker">✕</button></header><label class="picker-search">Search name, number or element<input id="picker-search" type="search" autocomplete="off" value="'+query.replace(/[&<>"]/g,'')+'" placeholder="e.g. Brimble #2"></label><div class="picker-filters">'+['All','Damage','Tank','Support'].map(r=>'<button data-picker-role="'+r+'" class="button secondary '+(role===r?'selected':'')+'" aria-pressed="'+(role===r)+'">'+r+'</button>').join('')+'</div><div class="picker-grid">'+(options.allowEmpty&&page===0&&!query?'<button class="picker-card picker-empty" data-pick=""><span>✧</span><strong>Empty this slot</strong></button>':'')+shown.map(m=>{
...
dialog.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;
 ...
 if(b.hasAttribute('data-pick')){const id=b.dataset.pick;if(id&&!entries().some(m=>m.id===id))return;const chosen=options.onChoose;if(chosen&&chosen(id||null)===false){dialog.querySelector('#picker-feedback').textContent=P.error()||'Unable to select this companion.';return;}dialog.close();}
