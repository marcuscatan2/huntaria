...
function draw(focusSearch=false){
 ...
 dialog.innerHTML='<header><div><h2 id="picker-title">'+escape(options.title||'Choose your companion')+'</h2></div><button id="picker-close" class="text-button" aria-label="Close companion picker">✕</button></header><label class="picker-search">Search name or element<input id="picker-search" type="search" autocomplete="off" value="'+query.replace(/[&<>"]/g,'')+'" placeholder="e.g. Brimble"></label><div class="picker-filters">'+['All','Damage','Tank','Support'].map(r=>'<button data-picker-role="'+r+'" class="button secondary '+(role===r?'selected':'')+'" aria-pressed="'+(role===r)+'">'+r+'</button>').join('')+'</div><div class="picker-grid">'+(options.allowEmpty&&page===0&&!query?'<button class="picker-card picker-empty" data-pick=""><span>✧</span><strong>Empty this slot</strong></button>':'')+shown.map(m=>{
