...
function render(scope='class'){
 ...
 const nodeMarkup=n=>{
  ...
  const action=!owned?(scope==='class'?'SPECIALIZE FIRST':'SUMMON TO TRAIN'):!unlocked?mon?'UNLOCKS AT LV 31':'SPECIALIZE FIRST':locked?(n.requirement||'Requires '+parent.name):rank>=n.max?'MAX RANK':used>=points?'No points available':'Improve (1 point)';
