...
function render(scope='class'){
 ...
 return '<button class="tree-node '+(rank?'learned':locked?'locked':'available')+'" data-learn="'+n.id+'" '+(!owned||!unlocked||locked||rank>=n.max||used>=points?'disabled':'')+'><span>'+n.icon+'</span><strong>'+n.name+'</strong><small>'+escape(n.ranks?n.ranks[Math.min(rank,n.max-1)]:n.label)+'</small><b>Rank '+rank+' / '+n.max+'</b><em>'+(!owned?(scope==='class'?'SPECIALIZE FIRST':'SUMMON TO TRAIN'):!unlocked?mon?'UNLOCKS AT PLAYER LV 30':'SPECIALIZE FIRST':locked?(n.requirement||'Requires '+parent.name):rank>=n.max?'MAX RANK':used>=points?'No points available':'Improve · 1 point')+'</em></button>';
 }).join('')+'</div></section>';
}
