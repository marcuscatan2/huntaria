...
with sync_playwright() as pw:
    ...
    try:
        ...
        if not args.smoke:
            ...
            page.locator('.frame-destinations [data-menu-close]').click();page.clock.run_for(300)
            ...
            check('Mage victory shows the return banner',page.locator('#victory-banner').is_visible())
            ...
            page.locator('.frame-destinations [data-menu-close]').click()
            ...
            normal.locator('.recovery-close').click()
            normal.locator('#open-settings').click()
            check('Normal Settings exposes an explicit test-mode switch and its three benefits',normal.locator('#test-mode-switch').inner_text()=='Enable test mode' and all(text in normal.locator('#test-mode-settings').inner_text() for text in ['3×','healing','5×','separate saves']))
            normal_save=normal.evaluate('localStorage.getItem(BondProfile.KEY)')
            normal.locator('#test-mode-switch').click();normal.wait_for_function('!!window.BondApp&&BondProfile.TEST')
            check('Enabling test mode resumes its separate save with faster movement and combat controls',normal.evaluate("BondProfile.snapshot().character.name==='New Journey'&&BondRegion.inspect().speed===BondAtlas.BASE_SPEED*3&&!!BondProfile.testing") and normal.locator('#qa-speed-5').count()==1)
            check('Switching into test mode leaves the normal save unchanged',normal.evaluate("localStorage.getItem('bond-bolt-profile-v7')")==normal_save)
            normal.set_viewport_size({'width':390,'height':844})
            normal.locator('#open-settings').click()
            check('Phone Settings offers the return action without horizontal overflow',normal.locator('#test-mode-switch').inner_text()=='Return to normal mode' and normal.evaluate('document.querySelector("#settings-dialog").scrollWidth<=document.querySelector("#settings-dialog").clientWidth'))
            normal.screenshot(path=str(ARTIFACTS/f'test-mode-settings-{args.browser}.png'))
            normal.locator('#test-mode-switch').click();normal.wait_for_function('!!window.BondApp&&!BondProfile.TEST')
            check('Returning to normal mode restores its own character and normal movement',normal.evaluate("BondProfile.snapshot().character.name==='Normal Save'&&BondRegion.inspect().speed===BondAtlas.BASE_SPEED&&!BondProfile.testing") and normal.locator('#qa-speed-5').count()==0)
            for width in [320,390,600]:
                normal.set_viewport_size({'width':width,'height':844})
                check(f'{width}px normal Settings stays inside the header above the map',normal.locator('#open-settings').evaluate('(el)=>{const r=el.getBoundingClientRect(),h=el.closest("header").getBoundingClientRect();return r.top>=h.top&&r.bottom<=h.bottom&&el.contains(document.elementFromPoint(r.x+r.width/2,r.y+r.height/2));}'))
                normal.locator('#open-settings').click()
                check(f'{width}px test-mode switch has a separate full-height tap target',normal.locator('#test-mode-switch').evaluate('(el)=>{const r=el.getBoundingClientRect(),next=el.nextElementSibling.getBoundingClientRect();return r.height>=44&&r.bottom<=next.top&&r.right<=innerWidth;}'))
                normal.locator('#settings-close').click()
            normal.close()
    except Exception:
