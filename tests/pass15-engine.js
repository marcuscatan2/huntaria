...
window.runPass15Engine=()=>{
 ...
 for(const type of Object.keys(C.UNITS)){
  ...
  check('F006 '+type+' kit, portrait, innate and 18 nodes',()=>{const portrait=document.createElement('div');portrait.innerHTML=CharacterRig.art(type);const visual=type==='apprentice'?portrait.querySelector('canvas'):['hunter','swordsman'].includes(type)?portrait.querySelector('svg[data-character="'+type+'"] image[href]'):portrait.querySelector('img[src]');return u.skills.length>=5&&new Set(u.skills).size===u.skills.length&&(!isMon||!!C.PASSIVES[u.passive])&&T.nodes(type).length===18&&!!visual;});
