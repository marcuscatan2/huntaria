...
window.runPass15Engine=()=>{
 ...
 const make=(type='emberfox',profile=state(),weapon='dagger',monsterRules=1)=>{const build=G.defaultBuild();if(C.UNITS[type].role==='Trainer')build[0][0]=entry(type,weapon);else {build[0][1]=entry(type);if(type==='stonehorn')build[0][2]=entry('emberfox');}return new G.Battle(build,{profile,seed:42,monsterRules});};
 const ready=(b)=>{b.random=()=>.99;for(const u of b.units){u.position={x:50+(u.side?2:0),y:50};u.hp=u.maxHp=10000;u.passive=null;u.shield=0;u.growth.armor=0;u.hardDefense=0;u.effective={};}b.elements=false;};
 ...
 for(const [type,cat,attr,skill] of [['emberfox','melee','str','pounce'],['frostfang','ranged','dex','snipe'],['mage','magic','int','comet']]){
  for(const changed of ['str','dex','int'])check('F001 legacy '+cat+' classic '+changed+' scaling',()=>{
   const s=state();s.xp.emberfox=495000;s.trainerXP=R.threshold(60);s.attributes[changed]=31;s.attributes.leadership=20;
   const d=R.derived(type,s,C.UNITS[type],null,true),a=Object.fromEntries(['str','dex','int'].map(k=>[k,type==='mage'?s.attributes[k]:Math.floor(s.attributes[k]*.1)]));
   const value=cat==='magic'?a.int+(Math.floor(a.int/7)**2+Math.floor(a.int/5)**2)/2:cat==='ranged'?a.dex+Math.floor(a.dex/10)**2+Math.floor(a.str/5):a.str+Math.floor(a.str/10)**2+Math.floor(a.dex/5);
   const level=type==='mage'||type==='emberfox'?60:1,expected=C.UNITS[type].power*(1+.025*(level-1))+value;
   return near(d.factors[cat],expected/C.UNITS[type].power)&&d.power===Math.round(expected);
  });
  check('F001 legacy '+cat+' final skill damage includes classic stats',()=>{
   const s=state();s.xp.emberfox=495000;s.trainerXP=R.threshold(60);s.attributes[attr]=31;s.attributes.leadership=20;
   const b=make(type,s,'dagger',0),u=b.units.find(x=>!x.side&&x.type===type);ready(b);const target=b.target(u),before=target.hp;
   b.strike(u,target,C.SKILLS[skill].amount,skill,C.SKILLS[skill]);
   const level=type==='mage'||type==='emberfox'?60:1,flat=type==='mage'?31+36:3;
   return before-target.hp===Math.round(C.SKILLS[skill].amount*((C.UNITS[type].power*(1+.025*(level-1))+flat)/C.UNITS[type].power));
  });
 }
 check('F002 classic STR 9/10/19/20 and secondary DEX breakpoints',()=>[9,10,19,20].map(str=>R.classic({str,dex:5}).melee).join()=== [10,12,21,25].join());
 check('F002 classic MATK minimum and maximum use INT seven/five steps',()=>{const s=R.classic({int:35});return s.magicMin===60&&s.magicMax===84;});
 check('F002 magic basics retain exact MATK endpoints when displayed power rounds up',()=>{const s=state();s.attributes.int=15;const b=make('mage',s);ready(b);const u=b.trainer(0),target=b.target(u);return [0,.999999].every(roll=>{const before=target.hp;b.random=()=>roll;b.strike(u,target,u.power,'Magic basic');return before-target.hp===C.UNITS.mage.power+15+(roll?9:4);});});
 check('F002 AGI changes attack delay, not movement or cooldown',()=>{const s=state();s.attributes.agi=11;const d=R.derived('druid',s);return near(d.speed,100/(C.UNITS.druid.interval*.955))&&d.cooldown===0&&C.UNITS.druid.moveSpeed===make('druid',s).trainer(0).moveSpeed;});
 check('F002 DEX changes cast time independently of cooldowns',()=>{const s=state();s.attributes.dex=11;const low=R.derived('druid',s);s.trainerXP=R.threshold(60);s.attributes.dex=99;return low.cooldown===0&&R.derived('druid',s).cooldown===0&&R.castTime(3,0)===3&&R.castTime(3,75)===1.5&&R.castTime(3,150)===0&&R.castTime(3,200)===0;});
 check('F002 HIT/FLEE includes level, capped physical odds and unavoidable magic',()=>near(rules.dodgeChance({agi:40},{dex:10},'melee'),.5)&&near(rules.dodgeChance({agi:10000},{dex:0},'ranged'),.95)&&near(rules.dodgeChance({agi:20},{dex:100},'melee'),.05)&&near(rules.dodgeChance({agi:10},{dex:10},'melee',10,20),.1)&&rules.dodgeChance({agi:1000},{},'magic')===0);
 check('F002 classic VIT defense and recovery integer boundaries',()=>R.physicalDefense(99,0)===78&&R.physicalDefense(99,.999)===113&&R.hpRecovery(1000,25)===10&&R.classic({int:20,vit:19}).magicDefense===29);
 ...
 check('F002 six-second HP recovery does not trigger a heal passive',()=>{const b=make();ready(b);b.move=()=>{};const u=b.trainer(0);for(const x of b.units)x.actionRemaining=999;u.hp=9900;u.effective.vit=25;u.passive='moonward';for(let i=0;i<119;i++)b.step();const before=u.hp;b.step();return before===9900&&u.hp===9955&&near(u.regenBuffer,0)&&u.shield===0;});
 check('F002 recovery caps, pauses while moving and stops in Overcharge',()=>{const b=make();ready(b);b.move=()=>{};const u=b.trainer(0);for(const x of b.units)x.actionRemaining=999;u.hp=9999;u.regenBuffer=5.95;b.step();const full=u.hp;b.overcharge=true;u.hp=9900;u.regenBuffer=5.95;b.step();b.overcharge=false;u.moving=true;u.regenBuffer=5.95;b.step();return full===10000&&u.hp===9900&&u.regenBuffer===0;});
 ...
 check('F003 legacy array tree migration has a finite refund notice',()=>{const s=P.normalize({version:4,owned:['emberfox'],growth:{druid:['bond','might']}});return Object.keys(s.growth.druid).length===0&&!s.migration.includes('NaN');});
 check('F010 frozen legacy mastery modifies its named skill effect and cooldown',()=>{
  ...
  const b=new G.Battle(build,{profile:s,seed:42,monsterRules:0}),actor=b.units.find(x=>x.instanceId===id);ready(b);
 ...
 check('F004 nearest trainer is a valid default target; skills can bypass a nearer companion',()=>{const b=make(),fox=b.units[1],mon=b.units.find(u=>u.side===1&&u.type==='stormowl'),trainer=b.trainer(1);fox.position={x:50,y:50};trainer.position={x:51,y:50};mon.position={x:56,y:50};const first=b.target(fox)===trainer;trainer.position.x=60;return first&&b.target(fox)===mon&&b.priorityTarget(fox)===trainer;});
 for(const first of BondFormation.RANKS)for(const second of BondFormation.RANKS)for(const third of BondFormation.RANKS)check('F005 shared rows '+[first,second,third].join('/'),()=>{const s=state(),ranks=[first,second,third];s.formation=ranks;const b=make('emberfox',s),allies=b.team(0);return BondFormation.valid(ranks)&&new Set(allies.map(u=>u.position.x+':'+u.position.y)).size===3&&allies.every((u,i)=>u.position.x===BondFormation.POSITIONS[ranks[i]].x)&&BondFormation.assign(ranks,0,'front').slice(1).join()===ranks.slice(1).join();});
 ...
 for(const type of Object.keys(C.UNITS)){
  ...
  check('F006 '+type+' kit, portrait, innate and matching mastery nodes',()=>{const portrait=document.createElement('div');portrait.innerHTML=CharacterRig.art(type);const visual=type==='apprentice'?portrait.querySelector('canvas'):['hunter','swordsman'].includes(type)?portrait.querySelector('svg[data-character="'+type+'"] image[href]'):portrait.querySelector('img[src]');return u.skills.length>=5&&new Set(u.skills).size===u.skills.length&&(!isMon||!!C.PASSIVES[u.passive])&&T.nodes(type).length===(type==='apprentice'?6:C.CLASSES.includes(type)?15:isMon?24:18)&&!!visual;});
  for(const id of u.legacySkills||u.skills)check('F006 assignment '+type+'/'+id+' executes',()=>{const b=make(type,state(),['trailshot','trailaim'].includes(id)?'bow':'dagger'),actor=b.units.find(x=>x.type===type&&!x.side);ready(b);for(const x of b.units){x.hp=9000;x.status.burn={until:10,source:actor.id,next:1};x.status.slow={until:10,source:actor.id,next:1};}actor.passive=u.passive;const accepted=b.cast(actor,C.SKILLS[id]);return accepted&&actor.casts===1&&b.units.every(x=>Number.isFinite(x.hp)&&x.hp>=0&&Number.isFinite(x.shield));});
  if(isMon){check('F006 '+type+' full battle finite',()=>{const b=make(type).run();return b.ended&&b.units.every(x=>Number.isFinite(x.hp)&&Number.isFinite(x.position.x)&&Number.isFinite(x.position.y));});speciesCoverage.push({id:type,role:u.role,element:u.element,passive:u.passive,assignments:[...u.skills],nodes:T.nodes(type).map(n=>n.id),source:A.home(type)?.map||'future-group:'+A.REGIONS[u.region].id,usefulPath:T.nodes(type).filter(n=>n.branchId==='A').slice(0,4).map(n=>n.id),artAcceptance:'owner review pending',bossOnlinePending:u.source==='boss'});}
  check('F010 '+type+' prerequisites, caps, usable branches',()=>{
   const cleaned=T.clean(type,{mastery:10,care2:5},20),all=T.clean(type,Object.fromEntries(T.nodes(type).map(n=>[n.id,999])),1000),nodes=T.nodes(type),hasHealing=u.skills.some(k=>['heal','teamheal','selfheal','cleanse'].includes(C.SKILLS[k].kind))||u.passive==='cinder';
   if(C.CLASSES.includes(type))return T.used(cleaned)===0&&T.used(all)===15&&T.used(T.clean(type,all,3))===3&&[...new Set(nodes.map(n=>n.branch))].every(branch=>{const path=nodes.filter(n=>n.branch===branch),ranks=T.clean(type,Object.fromEntries(path.map(n=>[n.id,999])),15);return T.used(ranks)===9&&path.every(n=>ranks[n.id]===n.max&&[1,2].includes(n.max)&&T.gate(type,n.id,ranks));});
   if(isMon){const legal=T.clean(type,Object.fromEntries(nodes.map(n=>[n.id,1])),15);return T.used(cleaned)===0&&T.used(all)===0&&nodes.length===24&&nodes.every(n=>n.max===1)&&T.used(legal)===15&&T.used(T.clean(type,legal,3))===3&&[...new Set(nodes.map(n=>n.branch))].every(branch=>{const path=nodes.filter(n=>n.branch===branch),ranks=T.clean(type,Object.fromEntries(path.map(n=>[n.id,1])),15);return T.used(ranks)===8&&path.every(n=>ranks[n.id]===1&&T.gate(type,n.id,ranks));});}
   return T.used(cleaned)===0&&nodes.every(n=>all[n.id]===n.max&&[3,5,10].includes(n.max)&&(n.stat!=='healing'||hasHealing))&&T.used(T.clean(type,all,3))===3;
  });
 }
 ...
 check('F007 guard applies element and overtime once',()=>{const b=make();ready(b);b.elements=true;b.overcharge=true;const actor=b.trainer(1),target=b.trainer(0),guard=b.units[2];actor.element='Fire';target.element='Earth';guard.element='Wind';guard.status.guard={until:10};b.damage(actor,target,100,'test');return target.hp===10000-156&&guard.hp===10000-84;});
 check('F007 Guard transfers post-defense HP without a second mitigation',()=>{const b=make();ready(b);const actor=b.trainer(1),target=b.trainer(0),guard=b.units[2];target.effective={int:20,vit:0};guard.effective={int:10,vit:40};guard.status.guard={until:10};b.damage(actor,target,100,'magic',true,{category:'magic'});return target.hp===9948&&guard.hp===9972;});
 check('F007 separate shields preserve their own strength and expiry',()=>{const b=make(),u=b.trainer(0);ready(b);u.pools=[];b.shield(u,u,200,10,'strong');b.time=1;b.shield(u,u,100,50,'weak');return u.shield===300&&u.pools.find(p=>p.label==='strong').until===10&&u.pools.find(p=>p.label==='weak').until===51;});
 ...
 check('F016 atlas 24 large maps / four interiors / six hubs / six boss domains / 94 wild species',()=>A.validate().length===0&&A.maps.filter(m=>!m.interior&&!['hub','boss'].includes(m.kind)).length===24&&A.maps.filter(m=>m.interior).length===4&&A.maps.filter(m=>m.kind==='hub').length===6&&A.maps.filter(m=>m.kind==='boss').length===6&&new Set(A.maps.flatMap(m=>m.habitats).map(h=>h.type)).size===94&&A.maps.flatMap(m=>m.habitats).length===99);
 for(const m of A.maps){
  ...
  if(!m.interior&&!['hub','boss'].includes(m.kind))check('F016 '+m.id+' shortest four directed edge walks >=30s',()=>{
 ...
 check('F016 all forty maps and interiors connected through physical gates',()=>{const seen=new Set(),queue=['clearing-0'];while(queue.length){const id=queue.shift();if(seen.has(id))continue;seen.add(id);queue.push(...A.get(id).neighbors.map(g=>g.to));}return seen.size===40;});
 ...
 check('P14 v6 migration preserves identity, XP, Echo and receipts while refunding retired nodes',()=>{
  ...
  return s.version===7&&s.companions.length===2&&a.id==='legacy:emberfox'&&a.xp===4500&&Object.keys(a.growth).length===0&&Object.keys(s.growth.druid).length===0&&s.coins===83&&s.inventory['echo:emberfox']===1&&s.summons['consumed-echo'].instanceId===a.id&&raw.version===6&&R.trainerLevel(s)===10;
 });
 ...
 return {results,speciesCoverage,counts:{species:C.MONSTERS.length,assignments:Object.values(C.UNITS).reduce((n,u)=>n+u.skills.length,0),innates:C.MONSTERS.length,treeNodes:T.TYPES.reduce((n,type)=>n+T.nodes(type).length,0),maps:A.maps.length},localOnly:true};
};
