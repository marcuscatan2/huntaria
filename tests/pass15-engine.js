...
window.runPass15Engine=()=>{
 ...
 const ready=(b)=>{b.random=()=>.99;for(const u of b.units){u.position={x:50+(u.side?2:0),y:50};u.hp=u.maxHp=10000;u.passive=null;u.shield=0;u.growth.armor=0;u.effective={};}b.elements=false;};
 ...
 for(const [type,cat,attr,skill] of [['emberfox','melee','str','pounce'],['frostfang','ranged','dex','snipe'],['mage','magic','int','comet']]){
  for(const changed of ['str','dex','int'])check('F001 '+cat+' classic '+changed+' scaling',()=>{
   const s=state();s.xp.emberfox=495000;s.trainerXP=R.threshold(60);s.attributes[changed]=31;s.attributes.leadership=20;
   const d=R.derived(type,s),a=Object.fromEntries(['str','dex','int'].map(k=>[k,type==='mage'?s.attributes[k]:Math.floor(s.attributes[k]*.1)]));
  ...
  check('F001 '+cat+' final skill damage includes classic stats',()=>{
   const s=state();s.xp.emberfox=495000;s.trainerXP=R.threshold(60);s.attributes[attr]=31;s.attributes.leadership=20;
   const b=make(type,s),u=b.units.find(x=>!x.side&&x.type===type);ready(b);const target=b.target(u),before=target.hp;
 ...
 check('P14 v6 migration retains XP, class/individual tree, Echo and receipts',()=>{
  ...
  return s.version===7&&s.companions.length===2&&a.id==='legacy:emberfox'&&a.xp===4500&&a.growth.might===1&&Object.keys(s.growth.druid).length===0&&s.coins===83&&s.inventory['echo:emberfox']===1&&s.summons['consumed-echo'].instanceId===a.id&&raw.version===6&&R.trainerLevel(s)===10;
 });
