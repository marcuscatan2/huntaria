...
with sync_playwright() as pw:
    ...
    try:
        ...
        check('No upgrade badges when every spendable point is used',not dot('[data-world-menu="collection"]') and not dot('[data-world-menu="collection"]'))
        ...
        check('A fully allocated trainer has no badge before leveling',not dot('[data-world-menu="collection"]'))
        page.evaluate('BondProfile.testing.setTrainerXP(BondProgress.threshold(59))')
        check('Trainer level-up immediately reveals new attribute and class points',dot('[data-world-menu="collection"]') and page.evaluate('()=>{const n=BondUpgradeNotices.read(BondProfile.snapshot());return n.stats.size>0&&n.trainer.available;}'))
        page.evaluate("""()=>{const s=upgradeFixture(),G=BondGrowth,type='acornboar';s.companions=[{id:'level-up',type,xp:BondProgress.threshold(30),skills:BondContent.UNITS[type].default,growth:{}}];s.companions[0].growth=G.clean(type,Object.fromEntries(G.nodes(type).map(n=>[n.id,1])),G.budget(s,'level-up'));BondProfile.testing.replace(s);}""")
        check('Level30 companion has no talent badge even with both quest rewards',not dot('[data-world-menu="collection"]'))
        page.evaluate("BondProfile.testing.setXP('level-up',BondProgress.threshold(31))")
        check('Level31 immediately reveals the companion talent route',dot('[data-world-menu="collection"]') and page.evaluate("BondUpgradeNotices.read(BondProfile.snapshot()).companions.get('level-up').available"))
        ...
        check('Unspent attributes mark Inner Sea and leave Bag clear',dot('[data-world-menu="collection"]') and not dot('[data-world-menu="inventory"]'))
        page.locator('[data-world-menu="collection"]').click()
        check('Inner Sea marks Attributes',dot('[data-sea-tab="trainer"]'))
        ...
        check('Spending the last affordable points clears the entire route',not dot('[data-frame-menu="collection"]') and not dot('[data-sea-tab="trainer"]') and page.locator('[data-stat].has-upgrade').count()==0)
        ...
        check('Class points mark Inner Sea, Trainer and its class tree',dot('[data-frame-menu="collection"]') and dot('[data-sea-tab="trainer"]') and dot('[data-collection-mode="trees"]'))
        ...
        check('Class spending clears the class route',not dot('[data-frame-menu="collection"]') and not dot('[data-collection-mode="trees"]'))
        ...
        page.locator('[data-world-menu="collection"]').click();page.locator('[data-sea-tab="party"]').click()
        check('Companion point routes mark My companions without unrelated Attributes',dot('[data-collection-mode="companions"]') and not dot('[data-sea-tab="trainer"]'))
        ...
        check('Companion purchase clears the last badge everywhere',not dot('[data-frame-menu="collection"]') and not dot('[data-frame-menu="collection"]'))
        ...
        check('Reload derives the cleared state from saved allocations',not dot('[data-world-menu="collection"]') and all(before[k]==after[k] for k in ['attributes','growth','companions','trainerXP','journey']))
        page.evaluate("()=>{const P=BondProfile,s=P.snapshot(),m=BondAtlas.get('clearing-hub'),r=m.residents.find(r=>r.service==='talents');s.map=m.id;s.position={x:r.x,y:r.y};P.testing.replace(s);P.testing.grantEcho('acornboar',1);P.resetCompanionTalents(r.id,'notice:20');BondApp.switchTab('loadout');BondTree.select('notice:20');}")
        check('An Echo-funded city reset immediately restores the individual point route',dot('[data-frame-menu="collection"]') and page.locator('.talent-node.has-upgrade').count()>0)
