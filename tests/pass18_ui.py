...
with sync_playwright() as pw:
    ...
    try:
        ...
        check('Atlas roads exactly represent reciprocal graph edges',page.evaluate("""()=>{const edges=new Set(BondAtlas.maps.filter(m=>!m.interior).flatMap(m=>m.neighbors.filter(g=>g.kind!=='stairs').map(g=>[m.id,g.to].sort().join('|'))));return document.querySelectorAll('[data-atlas-road]').length===edges.size&&[...document.querySelectorAll('[data-atlas-road]')].every(p=>edges.has(p.dataset.atlasRoad));}"""))
        ...
        page.locator('[data-object="clearing-hub:shop"]').click();page.clock.run_for(500);page.locator('[data-city-supplies]').last.click()
        ...
        check('Indoor city shop buys one cheap potion with earned hunt coins',page.evaluate('BondProfile.snapshot().coins')==before-3 and page.evaluate('BondProfile.snapshot().inventory.leafdraught')==potions+1)
        page.screenshot(path=str(ARTIFACTS/f'pass18-shop-ui-{args.browser}.png'),full_page=True);page.keyboard.press('Escape');page.keyboard.press('Escape')
        page.evaluate("""()=>{BondProfile.travel('clearing-0');const s=BondProfile.snapshot();s.vitality.trainer=5000;BondProfile.testing.replace(s);BondApp.switchTab('region');}""")
        ...
        check('Field UI applies potion and consumes exactly one',page.evaluate('BondAdventure.health(BondProfile.snapshot())')==min(10000,before+4590) and page.evaluate('BondProfile.snapshot().inventory.leafdraught')==potions)
        page.keyboard.press('Escape')
        before=page.evaluate('BondProfile.snapshot().coins')
        page.evaluate("""()=>{const s=BondProfile.snapshot();s.vitality.trainer=5000;BondProfile.testing.replace(s);const g=BondAtlas.get('clearing-0').neighbors.find(g=>g.to==='clearing-hub');BondProfile.position(g);if(!BondProfile.transition(g.id))throw Error('City entry failed');BondApp.switchTab('region');}""")
        page.clock.run_for(200)
        check('City arrival heals freely without a sanctuary interaction',page.evaluate('BondAdventure.health(BondProfile.snapshot())===10000&&BondApp.getTab()==="region"') and page.evaluate('BondProfile.snapshot().coins')==before and page.locator('.map-object.sanctuary').count()==0)
        ...
        page.locator('[data-object="early:master:druid"]').click();page.clock.run_for(300)
        ...
        page.locator('#npc-transform').click();page.locator('#npc-transform').click();page.clock.run_for(300)
