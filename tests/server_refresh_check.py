...
def main():
    ...
    try:
        with sync_playwright() as pw:
            ...
            check('Both bottom-menu destinations show the new icons and red dots', page.locator('#world-action-menu .destination-icon').count() == 3 and page.locator('#world-action-menu .upgrade-dot').count() == 2)
            ...
            check('Bag routes badges to both class talents and Inner Sea', page.locator('.menu-nav [data-menu="collection"] > .upgrade-dot').count() == 1 and page.locator('.menu-nav [data-menu="trees"] > .upgrade-dot').count() == 1)
            page.locator('.menu-nav [data-menu="collection"]').click()
            check('Inner Sea routes badges to attributes and companion talents', page.locator('[data-collection-mode="trainer"] > .upgrade-dot').count() == 1 and page.locator('[data-collection-mode="companions"] > .upgrade-dot').count() == 1)
