...
def main():
    ...
    try:
        with sync_playwright() as pw:
            ...
            check('Repeated map changes retain bounded textures and visible props', page.evaluate("""()=>{const s=WorldRenderer.inspect();return s.chunkCount<=28&&s.sheetCount<=2&&s.landscapeSheetCount===2&&s.decodedLandscapeBytes<=13*1024*1024&&document.querySelectorAll('.world-prop').length<200;}"""))
