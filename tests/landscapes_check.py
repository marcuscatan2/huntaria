...
def main():
    ...
    try:
        with sync_playwright() as pw:
            ...
            page.clock.run_for(100)
            # Aim at the visible destination label; frozen wildlife may cover the gate's center.
            label=page.locator('[data-object="clearing-0>clearing-3"] .world-label').bounding_box()
            page.mouse.click(label['x']+label['width']/2,label['y']+label['height']/2)
            page.clock.run_for(100)
            ...
            label=page.locator('[data-object="clearing-0>clearing-3"] .world-label').bounding_box()
            page.mouse.click(label['x']+label['width']/2,label['y']+label['height']/2);page.clock.run_for(100)
