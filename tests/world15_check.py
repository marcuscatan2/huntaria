...
with sync_playwright() as pw:
    ...
    try:
        ...
        page.screenshot(path=str(ARTIFACTS/'pass15-firstlight.png'),full_page=True)
        ...
        (ARTIFACTS/('pass15-nav-'+args.browser+'.json')).write_text(json.dumps(nav,indent=2),encoding='utf-8')
        for m in nav:
        ...
        for region in ['clearing','brook','hollow','ruins','rise','ashen']:
            for suffix in ['0','1','2','3','hub']:
                ...
                if suffix=='0' or region=='clearing':page.screenshot(path=str(ARTIFACTS/('pass15-'+target+'.png')),full_page=True)
                if suffix=='2':
                    ...
                    page.clock.run_for(200)
                    page.screenshot(path=str(ARTIFACTS/('pass15-hero-'+region+'.png')),full_page=True)
        ...
        page.screenshot(path=str(ARTIFACTS/'pass15-mobile.png'),full_page=True)
        ...
        fallback.goto(url);fallback.wait_for_function('!!window.BondApp&&WorldRenderer.inspect().fallback')
        ...
        if args.performance_seconds:
            ...
            (ARTIFACTS/('pass15-performance-'+args.browser+'.json')).write_text(json.dumps(perf,indent=2),encoding='utf-8')
    ...
    (ARTIFACTS/('pass15-'+args.browser+'.json')).write_text(json.dumps(report,indent=2),encoding='utf-8')
...
if args.full:
    sys.argv=['tests/pass15_regression.py','--browser',args.browser]
    runpy.run_path(str(ROOT/'tests/pass15_regression.py'),run_name='__main__')
