"""Pass 15 instance/picker/loot journeys in isolated browser contexts."""
...
with sync_playwright() as pw:
    ...
    try:
        ...
        source=(ROOT/'tests'/'pass15-engine.js').read_text(encoding='utf-8')
        page.add_script_tag(content=source)
        engine=page.evaluate('runPass15Engine()')
        ...
        page.screenshot(path=str(ARTIFACTS/'pass16-regression-combat.png'),full_page=True)
        ...
        page.screenshot(path=str(ARTIFACTS/'pass16-regression-picker.png'),full_page=True)
        ...
        page.screenshot(path=str(ARTIFACTS/'pass16-regression-inner-sea.png'),full_page=True)
        ...
        page.locator('[data-object="'+spawn+'"]').click();page.clock.run_for(6000);page.locator('#npc-fight').click()
        ...
        page.screenshot(path=str(ARTIFACTS/'pass16-regression-loot.png'),full_page=True)
        ...
        manifest=page.evaluate('''()=>({version:15,species:BondRoster.manifest().map(u=>({...u,habitat:BondAtlas.home(u.id),passiveInfo:BondContent.PASSIVES[u.passive],kit:u.skills.map(id=>({id,...BondContent.SKILLS[id]}))})),maps:BondAtlas.maps.map(({scenery,collisionBuckets,obstacles,...m})=>m),regions:BondAtlas.REGIONS})''')
        manifest['source_sha256']=source_start
        (ARTIFACTS/'pass16-regression-reference.json').write_text(json.dumps(manifest,indent=2,ensure_ascii=False),encoding='utf-8')
        ...
        page.screenshot(path=str(ARTIFACTS/'pass16-regression-mobile-picker.png'),full_page=True)
    ...
    (ARTIFACTS/('pass16-regression-'+args.browser+'.json')).write_text(json.dumps(report,indent=2),encoding='utf-8')
